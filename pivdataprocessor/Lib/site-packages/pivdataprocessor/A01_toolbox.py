...
class WelfordStatisticsCalculator:
    ...
    def add_point(self, x) -> None:
        ...
        self.M3 += delta * delta_n2 * (self.n - 1) * (self.n - 2) - 3 * delta_n * self.M2
        self.M2 += term1
    ...
    def get_skewness(self):
        """
        Return population-style skewness (no bias correction),
        matching scipy.stats.skew(..., bias=True)
        """
        if self.n < 3:
            return float('nan')
        n = self.n
    ...
    def get_flatness(self):
        """
        Return population-style kurtosis (Pearson definition, i.e. 3 for normal),
        matching scipy.stats.kurtosis(..., fisher=False, bias=True)
        """
        if self.n < 4:
            return float('nan')
        n = self.n
...
class np_fft():
    ...
    def fft(self, ifhamming = False):
        
        if ifhamming:
            window = np.hamming(len(self.signal))
            window /= np.sqrt(np.mean(window**2))  # 保持能量不变
            windowed_signal = self.signal * window
            result = np.fft.fft(windowed_signal)
        else:
...
class np_fft_normalized():
    def __init__(self, signal: np.ndarray, dx_in_mm: float, rms: np.ndarray):
        assert isinstance(signal, np.ndarray)
        assert signal.ndim == 1
        self.signal = signal
        self.dx_in_m = dx_in_mm / 1000
        self.rms = rms
        assert len(self.rms) == len(signal), "RMS array must have same length as signal" 
        self.result = np.zeros(signal.shape)
        self.wavenumber = np.zeros(signal.shape)
    
    def fft(self):
        signal_to_fft = self.signal.copy()
        signal_to_fft = np.divide(signal_to_fft, self.rms, 
                                    out=np.zeros_like(signal_to_fft), 
                                    where=self.rms!=0)

        result = np.fft.fft(signal_to_fft)
        self.result = np.abs(result)**2 / (np.pi * len(signal_to_fft)) * self.dx_in_m
        self.wavenumber = np.fft.fftfreq(len(signal_to_fft), d=self.dx_in_m) * 2 * np.pi

    def get_result(self):
        n = len(self.signal)
        return self.result[1:n//2], self.wavenumber[1:n//2]
...
def lagrange_interpolate_2D_4points_scalar(grid1:np.ndarray, grid2: np.ndarray, scalar1: np.ndarray) -> np.ndarray:
    ...
    Nx2, Ny2 = x.shape
    scalar2 = np.full(x.shape, np.nan, dtype=np.float64)

    for i in range(Nx2):
        for j in range(Ny2):
            i_ip = int((x[i,j] - x_read[0,0]) / dx + 1)
            j_ip = int((y[i,j] - y_read[0,0]) / dy + 1)

            if i_ip-2 < 0 or i_ip+1 >= Nx or j_ip-2 < 0 or j_ip+1 >= Ny:
                continue  

            x1, x2, x3, x4 = x_read[i_ip-2, j_ip], x_read[i_ip-1, j_ip], x_read[i_ip, j_ip], x_read[i_ip+1, j_ip]
            y1, y2, y3, y4 = y_read[i_ip, j_ip-2], y_read[i_ip, j_ip-1], y_read[i_ip, j_ip], y_read[i_ip, j_ip+1]

            ax1 = (x[i,j]-x2)*(x[i,j]-x3)*(x[i,j]-x4)/((x1-x2)*(x1-x3)*(x1-x4))
            ax2 = (x[i,j]-x1)*(x[i,j]-x3)*(x[i,j]-x4)/((x2-x1)*(x2-x3)*(x2-x4))
            ax3 = (x[i,j]-x1)*(x[i,j]-x2)*(x[i,j]-x4)/((x3-x1)*(x3-x2)*(x3-x4))
            ax4 = (x[i,j]-x1)*(x[i,j]-x2)*(x[i,j]-x3)/((x4-x1)*(x4-x2)*(x4-x3))

            by1 = (y[i,j]-y2)*(y[i,j]-y3)*(y[i,j]-y4)/((y1-y2)*(y1-y3)*(y1-y4))
            by2 = (y[i,j]-y1)*(y[i,j]-y3)*(y[i,j]-y4)/((y2-y1)*(y2-y3)*(y2-y4))
            by3 = (y[i,j]-y1)*(y[i,j]-y2)*(y[i,j]-y4)/((y3-y1)*(y3-y2)*(y3-y4))
            by4 = (y[i,j]-y1)*(y[i,j]-y2)*(y[i,j]-y3)/((y4-y1)*(y4-y2)*(y4-y3))
...
def nanmean_filter1d(data, kernel_size=5, kernel=None):
    ...
    summed = convolve(data_filled, kernel, mode='same', method='auto')
    counts = convolve(mask.astype(float), kernel, mode='same', method='auto')
...
def fit_2d_poly(X, field, order, region=(slice(None), slice(None))):
    result = np.full_like(field, np.nan)
    x_ravel = X[0][region].flatten() 
    y_ravel = X[1][region].flatten()
    X_mat = polyvander2d(x_ravel, y_ravel, [order, order])
    order1= order+1

    coeffs, *_ = lstsq(X_mat, field[region].flatten(), rcond=None)
    a_field = coeffs.reshape((order+1, order+1))
    result[region] = sum(a_field[i, j] * x_ravel**i * y_ravel**j for i in range(order1) for j in range(order1)).reshape(X[0][region].shape)
    return result
# -------------------------------------------------------------------------
# endregion
...
# region
class ProbabilityDensity_offline():
    '''Store all points first, then process histogram or KDE.'''
    def __init__(self):
        self.point_list = []
        self.ws = WelfordStatisticsCalculator()
        self.KDE_x = None
        self.KDE_y = None
    def add_point(self, points, if_cal_stats = False):
    ...
    def process(self, KDE_points = 1000, range = None):
        point_np = np.array(self.point_list)
        kde = gaussian_kde(point_np)
        if range is None:
            self.KDE_x = np.linspace(min(point_np), max(point_np), KDE_points)
        else:
            self.KDE_x = np.linspace(range[0], range[1], KDE_points)
        self.KDE_y = kde(self.KDE_x)

    def process_hist(self, bins = 30):
        point_np = np.array(self.point_list)
        hist, bin_edges = np.histogram(point_np, bins=bins, density=True)
        self.KDE_x = (bin_edges[:-1] + bin_edges[1:]) / 2
        self.KDE_y = hist
...
class ProbabilityDensity():
    def __init__(self, bins=30, hist_range=None, if_cal_stat = False):
        """
        bins  : int, number of histogram bins
        range : (min,max), histogram range; required for online histogram
        """
        self.point_list = []
        self.if_cal_stat = if_cal_stat
        # --- For statistical accumulation ---
        if if_cal_stat:
            self.ws = WelfordStatisticsCalculator()
        ...
        if hist_range is None:
            raise ValueError("Online histogram requires a fixed range = (min,max)")
        # --- Precompute bin edges ---
        self.bin_edges = np.linspace(hist_range[0], hist_range[1], bins+1)
        self.hist_counts = np.zeros(bins, dtype=float)  # online updated histogram
    ...
    def add_point(self, points):
        ...
        if isinstance(points, np.ndarray):
            ...
            raise ValueError("Only numpy array or Python list accepted")

        pts = pts[np.isfinite(pts)]
        if pts.size == 0:
            return
        # Append to global list
        self.point_list.extend(pts.tolist())
        ...
        # --- ONLINE histogram update ---
        # np.digitize returns bin index (1..bins)
        bin_idx = np.digitize(pts, self.bin_edges) - 1  # shift to 0..bins-1
        ...
        for b in bin_idx:
            self.hist_counts[b] += 1

    # --------------------------------------------------------------
    def process_hist(self):
        """
        Convert counts → probability density.
        """
        total_points = np.sum(self.hist_counts)
        if total_points == 0:
            raise ValueError("No data added yet.")

        # Bin centers
        self.KDE_x = (self.bin_edges[:-1] + self.bin_edges[1:]) / 2

        # Convert to PDF
        bin_width = self.bin_edges[1] - self.bin_edges[0]
        self.KDE_y = self.hist_counts / (total_points * bin_width)

    def normalize(self):
        if not self.if_cal_stat:
            raise ValueError("Statistics not computed; cannot normalize.")
        self.KDE_x = (self.KDE_x - self.ws.get_mean())/np.sqrt(self.ws.get_variance())
        self.KDE_y = self.KDE_y * np.sqrt(self.ws.get_variance())
...
class WriteHandler():
    def __init__(self, header:list[str], comment:list[str] = None):
        self.header = header
        self.comment = comment

    def loaddata(self, data:list):
        assert len(self.header) == len(data), \
            'Number of data sequences must match the length of the header list.'
        lens = [len(arr) for arr in data]
        if len(set(lens)) != 1:
            raise ValueError(f"All data sequences must have the same length.")    
        self.data = data

    def write(self,filepath:str, fmt:str = '.6e'):
        '''
            fmt: 
                .6e -> scientific notation style for Fortran
                .6f -> floating number style
        '''
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            for i in range(nrows):
                row = [f"{self.data[j][i]:{fmt}}" for j in range(ncols)]
                f.write(",".join(row) + "\n")
