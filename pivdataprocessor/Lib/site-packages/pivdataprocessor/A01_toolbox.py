...
def lagrange_interpolate_2D_4points_scalar(grid1:np.ndarray, grid2: np.ndarray, scalar1: np.ndarray) -> np.ndarray:
    '''
    grid1: origin grid, grid2: target grid, scalar1: data value in the origin grid.

    ! Make sure grid1 and grid2 have the same size. 
    ...
    scalar2 = np.zeros((Nx,Ny),dtype=float_precsion)
    ...
    for j in range(Ny):
        for i in range(Nx):
            ...
            elif x[i] == x_read[i_ip,j_ip] and y[j] == y_read[i_ip,j_ip]:
                scalar2[i,j] = scalar1[i,j]
            
            elif x[i] == x_read[i_ip,j_ip] and y[j] != y_read[i_ip,j_ip]:
                by = (y_read[i_ip, j_ip]-y[j])/(y[1]-y[0])
                scalar2[i,j] = by * scalar1[i_ip,j_ip-1] + (1-by) * scalar1[i_ip,j_ip]
            
            elif x[i] != x_read[i_ip,j_ip] and y[j] == y_read[i_ip,j_ip]:
                ax = (x_read[i_ip, j_ip]-x[i])/(x[1]-x[0])
                scalar2[i,j] = ax * scalar1[i_ip-1,j_ip] + (1-ax) * scalar1[i_ip,j_ip]
...
def shift_field(field:np.ndarray, shift_x:int, shift_y:int, boundary_value = 0.0):
    '''
    Example:
    0 1 2 3  shift_x =  1   0 0 0 0  
    4 5 6 7  shift_y = -1   0 0 1 2
    7 8 9 10 ------------>  0 4 5 6
    '''
    # shifted_field = np.full(field.shape, boundary_value,dtype=float_precsion)
    shifted_field = np.zeros(field.shape)
