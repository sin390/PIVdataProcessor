...
comment_ratio = ['Note: these are interpolated values used for calculating the ratio.',
           ...
           '-- a: XY-plane Z = 0, u_rms(x, y=12, z=0)',
           '-- b: XZ-plane Y = 0, u_rms(x, y=0, z=12)']
ws = WriteHandler(['x(mm)','a/b','err(a/b)','-err(a/b)','a','err(a)','-err(a)','b','err(b)','-err(b)'],comment_ratio)
ws_filename = '\\ratio.txt'

comment_a = ['XY-plane Z = 0, u_rms(x, y=12, z=0)']
ws_a = WriteHandler(['x(mm)','u_rms','err(u_rms)','-err(u_rms)'], comment_a)
ws_a_filename = '\\XY-Z0, u_rms(x,y=12,z=0).txt'

comment_b = ['XZ-plane Y = 0, u_rms(x, y=0, z=12)']
ws_b = WriteHandler(['x(mm)','u_rms','err(u_rms)','-err(u_rms)'],comment_b)
ws_b_filename = '\\XZ-Y0, u_rms(x,y=0,z=12).txt'
