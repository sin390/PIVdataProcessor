...
comment_ratio = ['Note: these are interpolated values used for calculating the ratio.',
           ...
           '-- a: XY-plane Z = 0, u_rms(x, y=0, z=0)',
           '-- b: XZ-plane Y = 0, u_rms(x, y=0, z=0)']
ws = WriteHandler(['x(mm)','a/b','err(a/b)','a','err(a)','b','err(b)'],comment_ratio)
ws_filename = '\\ratio.txt'

comment_a = ['XY-plane Z = 0, u_rms(x, y=0, z=0)']
ws_a = WriteHandler(['x(mm)','u_rms','err(u_rms)'], comment_a)
ws_a_filename = '\\XY-Z0, u_rms(x,y=0,z=0).txt'

comment_b = ['XZ-plane Y = 0, u_rms(x, y=0, z=0)']
ws_b = WriteHandler(['x(mm)','u_rms','err(u_rms)'],comment_b)
ws_b_filename = '\\XZ-Y0, u_rms(x,y=0,z=0).txt'
...
rms_a = np.sqrt(rs.uu[left:right,line_y])
...
rms_b = np.sqrt(rs.uu[left:right,line_y])
...
plt.plot(x_a, rms_a,  label='urms_smoke')
# plt.plot(x_uniform, a_uniform, '-', label='interpolated v_rms')
plt.plot(x_b, rms_b,  label='urms_ethanol')
