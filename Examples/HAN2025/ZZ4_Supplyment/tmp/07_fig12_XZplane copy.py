...
ws = WriteHandler(['x(mm)','urms(m/s)','err_urms(m/s)','wrms(m/s)','err_wrms(m/s)'])
...
for case_number in range(len(cases)):
    ...
    ws.loaddata([plot_x,\
                 urms[left:right,central_y],urms_uncertainty[left:right,central_y]/2,\
                 vrms[left:right,central_y],vrms_uncertainty[left:right,central_y]/2])
    ws.write(fig_path+f'/urms_wrms_along_z=0_{cases[case_number]}.txt')
    plt.plot(ws.data[0],ws.data[1],label = f'urms {cases[case_number]}', color = mycolors[case_number])
    plt.errorbar(ws.data[0],ws.data[1],ws.data[2],fmt='none',ecolor=mycolors[case_number],elinewidth=0,capsize=5)
