...
for case_number in range(len(cases)):
    ...
    S11_sub1 = sub1.invariant_eta[left:right, bottom:up].copy()
    
    sub2 = RS(cases_sub2[case_number])
    sub2.load()
    S11_sub2 = sub2.invariant_eta[left:right, bottom:up].copy()

    uncertainty = np.sqrt(np.average((S11_sub1 - S11_sub2)**2))

    reporter.GTreport(f'Case: {cases[case_number]}', logfile = logfile)
    reporter.GTreport(f'Uncertainty of eta_avg : {uncertainty:.4f}', logfile = logfile)