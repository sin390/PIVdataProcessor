...
wh_x = WriteHandler(['k_x(m-1)','Eu(m3/s2)','k5.3*Eu(m4.3/s2)','err_Eu'])
wh_y = WriteHandler(['k_y(m-1)','Ev(m3/s2)','k5.3*Ev(m4.3/s2)','err_Ev'])
...
for case_number in range(len(cases)):
    ...
    wh_x.loaddata([es.wavenumber_xdir,es.spec_xdir[0],es.spec_xdir[0]*(es.wavenumber_xdir**target), spec_xdir_uncertainty])
    wh_x.write(fig_path+'/Eu'+f'/{cases[case_number]}_Eu.txt')
    wh_y.loaddata([es.wavenumber_ydir,es.spec_ydir[1],es.spec_ydir[1]*(es.wavenumber_ydir**target), spec_ydir_uncertainty])
...
wh_x.loaddata([k_x,Eu,Eu*(k_x**target),np.full_like(k_x, np.nan, dtype=float)])
wh_x.write(fig_path+'/Eu'+f'/Mori_Eu.txt')
wh_y.loaddata([k_y,Ev,Ev*(k_y**target),np.full_like(k_y, np.nan, dtype=float)])
