...
for i in range(2):
    for j in range(3):
        ...
        print(f'case_number={case_number}, u_rms_avg={u_rms_avg}')
        u_rms = np.sqrt(rs.uu)
        ...
        print(f'case_number={case_number}, v_rms_avg={v_rms_avg}')
        v_rms = np.sqrt(rs.vv)
        ...
        c = ax.imshow(relative_v_rms, extent=[X.min(), X.max(), Y.min(), Y.max()],
                    cmap='bwr', origin='lower', interpolation='None',
                    vmin=vmin, vmax=vmax)
        contours = ax.contour(X, Y, relative_v_rms, levels=[-0.15,-0.05,0.05,0.15], colors='black', linestyles = '-', linewidths=0.5)
        if case_number == 0:
            first_im[1] = c       

        print(f'case_number={case_number}, ratio={u_rms_avg/v_rms_avg}')  
