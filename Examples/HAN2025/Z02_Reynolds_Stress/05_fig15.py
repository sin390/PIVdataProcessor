...
colorbarlabels = [r'$R_{12}/R_{11}$']
figtitles = ['R12_R11_Contour']
figformat = '.pdf'

first_im = [None for _ in range(fig_number)]
N_R12_max = np.zeros(shape=(fig_number,6))
N_R12_min = np.zeros(shape=(fig_number,6))
filter_size = 8
for i in range(2):
    for j in range(3):
        case_number = i*3+j 
        ...
        N_R12 = R12/R11
        N_R12 = N_R12.T
        N_R12_max[fig_id,case_number] = np.nanmax(N_R12)
        N_R12_min[fig_id,case_number] = np.nanmin(N_R12)
...
for fig_id in range(fig_number):
    ...
    print(np.nanmax(N_R12_max[fig_id]))
    print(np.nanmin(N_R12_min[fig_id]))
    ...
    colorbar.ax.set_xticks([-0.15,-0.09,-0.03,0.03,0.09,0.15])
    fig = figs[fig_id]
