...
data_handler = WriteHandler(['x(mm)','u_avg(m/s)','err_u_avg(m/s)','-err_u_avg(m/s)'])
for case_number in range(len(cases)):
    ...
    for k in range(len(plotted_y)):
        ...
        yerr = U_uncertainty[left:right, plotted_y[k]] / 2
        data_handler.loaddata([plot_x,plot_y, yerr, -yerr])
        data_handler.write(case_dir+f'/y={round(pBase.X[1][0,plotted_y[k]])}.txt')
