...
class AutoCorrelation(pTS):
    def __init__(self, casename, avg_ylines_for_x = None, avg_xlines_for_y = None, center = 'Grid'):
        ...
        if center == 'Flow':
            self.stepping_indexs_x = central_stepping_indexs(self.ac_length_xdir) + self.CaseInfo.Central_Position_Flow[0]
            self.stepping_indexs_y = central_stepping_indexs(self.ac_length_ydir) + self.CaseInfo.Central_Position_Flow[1]
        elif center == 'Grid':
            self.stepping_indexs_x = central_stepping_indexs(self.ac_length_xdir) + self.CaseInfo.Central_Position_Grid[0]
            self.stepping_indexs_y = central_stepping_indexs(self.ac_length_ydir) + self.CaseInfo.Central_Position_Grid[1]            
    ...
    def calculate(self, start_value =0.5, end_value = 1e-5):
        ...
        self.__fitting(start_value,end_value) 
        self.__save()

    def __fitting(self, start_value = 0.5, end_value = 0.00001):
        self.result_json.set(1, start_value=start_value, end_value=end_value)
        def fittingpart(f,r):
            log_f = np.log(f)
            if len(r) == 0:
                return
        ...
        if (self.autocorr_xdir[0][-1] > end_value and self.autocorr_xdir[0][-1]<start_value):
            half_pos_x = np.abs(self.autocorr_xdir[0] - start_value).argmin()
            ...
            self.result_json.set(0,L11_x=L11_x, L11_x_measured_part = L11_x_measure, L11_x_fitted_part = L11_x_fitting)
            ...
            self.result_json.set(0,L11_x=L11_x_measure, L11_x_measured_part = L11_x_measure, L11_x_fitted_part = 0)
            L11_x = L11_x_measure

        if (self.autocorr_ydir[1][-1] > end_value and self.autocorr_ydir[1][-1] < start_value):
            half_pos_y = np.abs(self.autocorr_ydir[1] - start_value).argmin()
            ...
            self.result_json.set(0,L22_y=L22_y, L22_y_measured_part = L22_y_measure, L22_y_fitted_part = L22_y_fitting)
            ...
            self.result_json.set(0,L22_y=L22_y_measure, L22_y_measured_part = L22_y_measure, L22_y_fitted_part = 0)
            L22_y = L22_y_measure
        self.result_json.set(0,L11_L22 = L11_x/L22_y)
