...
class PDF_for_IS(GT):
    ...
    def calculate(self,bins=100, hist_range_coef_to_avg=None, coef_threshold = 2):
        ...
        self.PDF_x = x
        self.PDF_y = y
        np.save(result_path + "/PDF_for_ISH_x.npy", x)
        np.save(result_path + "/PDF_for_ISH_y.npy", y)
 
    def load_result(self, coef_threshold = 2):
        result_path = self.result_path + self.td.vfh.middle_path()
        result_path = result_path+ f'/Cth_{int(coef_threshold*1000)}'
        self.PDF_x = np.load(result_path + "/PDF_for_ISH_x.npy")
        self.PDF_y = np.load(result_path + "/PDF_for_ISH_y.npy")
