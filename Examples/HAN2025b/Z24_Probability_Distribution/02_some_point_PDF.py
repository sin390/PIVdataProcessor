...
from pivdataprocessor.A01_toolbox import ProbabilityDensity_offline as PD
...
import ZZZ_Result_Manager.A01_cases as A01_cases
...
cases = A01_cases.cases_w
cases_title = ['Case 1', 'Case 2', 'Case 3', 'Case 4', 'Case 5']
...
for case_number in range(len(cases)):
    ...
    for run_id in range(len(pBase.frame_numbers_in_runs)):
        for frame_id in range(pBase.frame_numbers_in_runs[run_id]):
            ...
            for tmp_i in range(len(PDF_u)):
                PDF_u[tmp_i].add_point([pBase.fluc_U[0][plotted_x[tmp_i],plotted_y[tmp_i]]],if_cal_stats=True)
                PDF_v[tmp_i].add_point([pBase.fluc_U[1][plotted_x[tmp_i],plotted_y[tmp_i]]],if_cal_stats=True)
    for tmp_i in range(len(PDF_u)):
        PDF_u[tmp_i].process()
    ...
    pos_number = 0          
    'fig1'
    axess[0][pos_number].plot(PDF_u[0].KDE_x, PDF_u[0].KDE_y, linestyle='-', color = mycolors[case_number],\
                                label = cases_title[case_number])
    'fig2'
    axess[1][pos_number].plot(PDF_v[0].KDE_x, PDF_v[0].KDE_y, linestyle='-', color = mycolors[case_number],\
                                label = cases_title[case_number])     

cases = A01_cases.cases_f
