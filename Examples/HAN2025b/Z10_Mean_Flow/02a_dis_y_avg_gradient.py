...
for case_id, case in enumerate(cases):
    ...
    x = pBase.X[1][i_x[0],bottom:up]
    y = fs.fit_avg_dUdX[0][1][i_x[0],bottom:up]*1000
    fig.plot(fig_id,x,y,color=colors[case_id])
    
    x = pBase.X[1][i_x[1],bottom:up]
    y = fs.fit_avg_dUdX[0][1][i_x[1],bottom:up]*1000
    fig.plot(fig_id,x,y,color=colors[case_id], linewidth = 0.5, linestyle='-.')  
...
for case_id, case in enumerate(cases):
    ...
    x = pBase.X[1][i_x[0],bottom:up]
    y = fs.fit_avg_dUdX[1][0][i_x[0],bottom:up]*1000
    fig.plot(fig_id,x,y,color=colors[case_id])
    
    x = pBase.X[1][i_x[1],bottom:up]
    y = fs.fit_avg_dUdX[1][0][i_x[1],bottom:up]*1000
    fig.plot(fig_id,x,y,color=colors[case_id], linewidth = 0.5, linestyle='-.') 
...
fig.set_label(2,xlabel=r'$y~\mathrm{(mm)}$')
fig.set_label(3,xlabel=r'$y~\mathrm{(mm)}$')
fig.set_axis(0,xlim=(-40,40),ylim=(-30,10))
fig.set_axis(1,xlim=(-40,40),ylim=(-10,30))
fig.set_axis(2,xlim=(-40,40),ylim=(-20,20))
fig.set_axis(3,xlim=(-40,40),ylim=(-10,30))
