...
for curve_id, filter_param in enumerate(plotted_filter_param):
    ...
    x = sl.X_BRF[1,ic,:]/Lf
    y = mag[ic,:]
    fig.plot(fig_id,x,y, linestyle = linestyles[curve_id], color=colors[curve_id])


for i in range(2):
    fig.set_panel_label(i)
fig.set_axis(0,xlim=(-3,3),ylim=(-0.3,1.1))
fig.set_axis(1,xlim=(-3,3),ylim=(-0.3,1.1))
...
fig.set_label(1,labelpad=15, ylabel=r'$\overline{\widetilde{\omega}_S}/ (\overline{\widetilde{\omega}_S})_0$')
# ---------------------------
