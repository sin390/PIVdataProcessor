...
from Z31_Triple_Decomposition.G01_triple_decomposition import TripleDecomposition as TD
from Z32_Shear_Layer.T01_local_toolbox import compute_mask_lagrange, lagrange_interpolate_with_mask
    ...
    for coeff_id, coeff in enumerate(coeffs_to_eta):
        ...
        with Pool() as pool:
            ...
            for case_id, case in enumerate(cases):
                ...
                Lf = eta * coeff
                tasks.append((cases[case_id], filter, coeff_id, Lf, cth, None))
        ...
        for deg in degs:
            with Pool() as pool:
                ...
                for case_id, case in enumerate(cases):
                    ...
                    eta = rm.result_table.get(1)['eta']*1000
                    Lf = eta * coeff
                    tasks.append((cases[case_id], filter, coeff_id, Lf, cth, deg))
                results = pool.map(worker, tasks)  
