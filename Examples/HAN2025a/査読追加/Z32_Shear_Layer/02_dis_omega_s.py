...
for case_id, case in enumerate(cases_select):
    ...
    x = sl.X_BRF[1,ic,:]/Lf
    y = sl.avg_u_BRF[0][jc,:]/sl.result_json.get(0)['jump_u']
    fig.plot(fig_id,x,y,color=colors[case_id])
fig.plot(fig_id,[-0.25,0.25],[0.5,0.5], color='k')
fig.plot(fig_id,[-0.25,0.25],[-0.5,-0.5], color='k')

fig.set_label(0,xlabel=r'$\boldsymbol{\zeta}_{y}/L_{F}$')
fig.set_label(0,ylabel=r'$<\widetilde{\omega}_S>_\mathrm{norm}$')
fig.set_label(1,xlabel=r'$\boldsymbol{\zeta}_{y}/L_{F}$')
fig.set_label(1,ylabel=r'$<\widetilde{u}_{\zeta_x}>/\Delta u$')
fig.set_axis(0,xlim=(-5,5),ylim=(-1.1,1.1))
fig.set_axis(1,xlim=(-5,5),ylim=(-1.1,1.1))
for fig_id in range(2):
    fig.set_panel_label(fig_id)
fig.set_margins(right=0.8)
fig.legend(bbox_to_anchor=(0.7,0.5))
fig.save(getplotpath()+"/dis_omega_s.png")
