...
for case_id, case in enumerate(A01.cases[:-1]):
    ...
    for filter_id, filter_param in enumerate(A01.gaussian_id):
        ...
        x.append(Lf/L11)
        y.append(avg*(Lf)*(Lf/L11)**(-1/3))

    fig.plot(fig_id,x,y,label=A01.case_labels[case_id],marker='^',markersize = 5,ifmarker=True, color=colors[case_id])
...
for case_id, case in enumerate(A01.cases[:-1]):
    ...
    for filter_id, filter_param in enumerate(A01.gaussian_id):
        ...
        x.append(Lf/L11)
        y.append(avg*(Lf)*(Lf/L11)**(-1/3))
    fig.plot(fig_id,x,y,marker='^',markersize = 5,ifmarker=True, color=colors[case_id])
...
fig.set_axis(0,ylim=(0,50),xlim=(0,0.4))

fig.set_axis(1,ylim=(0,15),xlim=(0,0.4))
fig.set_label(0,xlabel=r'$L_f/L_{u_1}$')
fig.set_label(1,xlabel=r'$L_f/L_{u_1}$')
# fig.set_label(2,xlabel=r'$L_f/\eta$')
fig.set_label(0,ylabel=r'$(\widetilde{I}_\mathrm{S}L_f)_\mathrm{avg} (L_f L_{u_1}^{-1})^{-1/3}$',labelpad= 15)
fig.set_label(1,ylabel=r'$(\widetilde{I}_\mathrm{R}L_f)_\mathrm{avg}(L_f L_{u_1}^{-1})^{-1/3}$',labelpad= 15)
