...
from pivdataprocessor.A01_toolbox import scalar_field_5points_stencil as derivative
...
class TripleDecomposition(GH):
    ...
    def cal_avg(self, order = 3):
        ...
        for run_id in range(len(self.frames_in_runs)):
            for frame_id in range(self.frames_in_runs[run_id]):
                ...
                swc_shear.add_point(self.intensity_shear**order)
                swc_rotation.add_point(self.intensity_rotation**order)
                swc_elongation.add_point(self.intensity_elongation**order)
                swc_mag_omega.add_point(np.abs(self.omega)**order)
    ...
    def cal_and_identify_SH_layer_frame(self, run_id, frame_id, coef_threshold = 1.5):
