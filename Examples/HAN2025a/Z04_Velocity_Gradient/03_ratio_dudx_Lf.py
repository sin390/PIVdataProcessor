...
for case_id, case in enumerate(A01.cases):
    ...
    fig.plot(fig_id,x,y,marker='^',markersize = 5,ifmarker=True, color=colors[case_id],xlog=True)
...
for case_id, case in enumerate(A01.cases):
    ...
    fig.plot(fig_id,x,y,label=A01.case_labels[case_id],marker='^',markersize = 5,ifmarker=True, color=colors[case_id],xlog=True)
...
for case_id, case in enumerate(A01.cases):
    ...
    for filter_id, filter_param in enumerate(A01.gaussian_id):
        x.append(2*np.pi/A01.Lf_coeff[filter_id])
        dr = DR(case, filter, filter_param)
        y.append(dr.result_json.get(0)['dudy_rms']/dr.result_json.get(0)['dvdx_rms'])
    fig.plot(fig_id,x,y,marker='^',markersize = 5,ifmarker=True, color=colors[case_id],xlog=True)
